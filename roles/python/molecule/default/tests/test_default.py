...
def test_pipsi_installed(host):
    with host.sudo():
        host.run_expect([0], 'test -e /home/example/.local/bin/pipsi')
        host.run_expect([0], 'test -L /home/example/.local/bin/pipsi')


def test_glances_installed(host):
    with host.sudo():
        glances_file = host.run('test -e /home/example/.local/bin/glances').rc
        glances_symlink = host.run('test -L /home/example/.local/bin/glances').rc
...
def test_pyenv_installed(host):
    with host.sudo():
        pyenv_file = host.run('test -e /home/example/code/personal/pyenv/bin/pyenv').rc
